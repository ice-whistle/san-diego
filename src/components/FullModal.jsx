...
export const FullModal = ({modal, setModal, language, setModalContent, modalContent}) =>{
  ...
  return (
    <Modal
        open={modal}
        onClose={()=>{setModal(null); setModalContent(null);}}
        ...
        !modalContent ?
          <Card elevation={3} className='modalCard'>
              {(modalDetail[index].titleEN || modalDetail[index].detailsEN) && <CardContent sx={{display: 'flex', flexDirection: 'column', gap: '16px'}}>
                  ...
                  {modalDetail[index].detailsEN && <Typography variant='body1' sx={{textAlign: 'left', whiteSpace: 'pre-wrap', color: color.dark}}>{language === 'EN' ? modalDetail[index].detailsEN : modalDetail[index].detailsES}</Typography>}
              </CardContent>}
              {(modalDetail[index].buttonLabel1EN || modalDetail[index].buttonLabel2EN) && <CardActions sx={{display: 'flex', padding: '0 16px 16px 16px'}}>
                  {modalDetail[index].buttonLabel2EN && <Button onClick={()=>setModal(null)} color={priority === 'high' ? 'secondary' : 'primary'} target="_blank" variant="outlined" size='large' sx={{width: '100%', height: '100%'}}>{language === 'EN' ? modalDetail[index].buttonLabel2EN : modalDetail[index].buttonLabel2ES}</Button>}
                  {modalDetail[index].buttonLabel1EN && <Button href={language === 'EN' ? modalDetail[index].link1EN : modalDetail[index].link1ES} color={priority === 'high' ? 'secondary' : 'primary'} startIcon={modalDetail[index]?.iconType === 'start' ? modalDetail[index].iconName : ''} endIcon={modalDetail[index]?.iconType === 'end' ? modalDetail[index].iconName : ''} target="_blank" variant="contained" size='large' sx={{width: '100%', minWidth: 'auto', whiteSpace: 'nowrap'}}>{language === 'EN' ? modalDetail[index].buttonLabel1EN : modalDetail[index].buttonLabel1ES}</Button>}
              </CardActions>}
          </Card> :
          <Card elevation={3} className='modalCardImage'>
            {modalContent}
          </Card>
        }
    </Modal>
